...
public class XJDealDetailsActivity extends BaseMvpActivity<XJActivityRequestView, XJDealDetailsRequestPresenter>
    ...
    public void initData(ListBean b) {
        ...
        if (!AppTool.isNull(bv.isSjczFj) && "1".equals(bv.isSjczFj) && !AppTool.isNull(bv.sSjczId)) {
            presenter.RequestFileDetailsPhoto(202, bv.sSjczId);
        }
        if (!AppTool.isNull(bv.isTd) && "1".equals(bv.isTd)) {
            presenter.RequestTDlist(bv.sMangeId);
        }
